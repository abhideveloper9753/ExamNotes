...
function FinalResult({ result }) {
    ...
    const data = typeof result === "string" ? { notes: result } : result;

    const hasSubTopics = data.subTopics && Object.keys(data.subTopics).length > 0;
    const hasQuestions = data.questions && ((data.questions.short && data.questions.short.length) || (data.questions.long && data.questions.long.length));
    const hasRevision = data.revisionPoints && data.revisionPoints.length;
    const hasNotesText = !!(data.notes && data.notes.length);
    const hasDiagram = data.diagram && data.diagram.data;
    const hasCharts = data.charts && data.charts.length > 0;

    if (!(hasSubTopics || hasQuestions || hasRevision || hasNotesText || hasDiagram || hasCharts)) {
        return null;
    }
        ...
        <div className='mt-6 p-3 space-y-10 bg-white'>
          ...
          bg-clip-text text-transparent'>
            ...
            {!quickRevision && <section>
                ...
                {hasSubTopics && Object.entries(data.subTopics).map(([star, topics]) => (
